...
// DB-Pfad: erst /data (Railway Volume), sonst /tmp (fallback), sonst lokal
function getDbPath() {
  const candidates = [
  ...
  for (const dir of candidates) {
    try {
      ...
      console.log(`✓ Datenbank-Verzeichnis: ${dir}`);
      return path.join(dir, 'einweisungen.db');
    } catch (e) {
      console.log(`✗ ${dir} nicht beschreibbar: ${e.message}`);
    }
  }
  throw new Error('Kein beschreibbares Verzeichnis für die Datenbank gefunden.');
}
...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS maschinen (
    id TEXT PRIMARY KEY, label TEXT, serial TEXT, year TEXT,
    type TEXT, groups TEXT, builtin INTEGER DEFAULT 0
  );
